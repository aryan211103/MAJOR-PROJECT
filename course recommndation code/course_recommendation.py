...
# Load the dataset containing course information
courses_df = pd.read_csv('all_courses_updated_dataset.csv')

# List of allowed courses for the dropdown
allowed_courses = [
    'DevOps', 'Mobile app development', 'Cybersecurity', 'IOT',
    'Software Engineering', 'Data Science', 'Cloud Computing',
    'Machine Learning', 'AI', 'UI/UX design', 'Blockchain', 'Web Development'
]

# Filter dataset to only include allowed courses
course_names = [course for course in allowed_courses if any(courses_df['course_name'].str.contains(course, case=False))]

# Sample questions for different courses (DevOps as an example)
question_db = {
    'DevOps': {
        'beginner': [
...
# Function to recommend the learning pathway based on skill level
def recommend_learning_pathway(user_skill_level, course_interest):
    course_modules = courses_df[courses_df['course_name'].str.contains(course_interest, case=False)]

    if user_skill_level.lower() == 'beginner':
        recommended_modules = course_modules[course_modules['difficulty_level'].isin(['Easy', 'Medium', 'Hard'])]
    elif user_skill_level.lower() == 'intermediate':
        recommended_modules = course_modules[course_modules['difficulty_level'].isin(['Medium', 'Hard'])]
    elif user_skill_level.lower() == 'expert':
        recommended_modules = course_modules[course_modules['difficulty_level'] == 'Hard']
    else:
        return "Invalid skill level. Please choose Beginner, Intermediate, or Expert."

    difficulty_order = {'Easy': 1, 'Medium': 2, 'Hard': 3}
    recommended_modules['difficulty_order'] = recommended_modules['difficulty_level'].map(difficulty_order)
    recommended_modules = recommended_modules.sort_values('difficulty_order')

    # Structure the output nicely
    return recommended_modules[['module', 'difficulty_level', 'link']]

# Function to ask multiple-choice questions and score the user
def ask_questions(course_interest, num_questions=3):
    course_questions = question_db.get(course_interest)
    ...
    for level in ['beginner', 'intermediate', 'expert']:
        questions = course_questions[level]
        num_to_ask = min(num_questions, len(questions))

        if num_to_ask == 0:
            continue

        selected_questions = random.sample(questions, num_to_ask)
        total_questions += num_to_ask

        for q in selected_questions:
            question_text.set(f"Question: {q['question']}")
            options_list.set(f"Options: a) {q['options']['a']}, b) {q['options']['b']}, c) {q['options']['c']}")
            root.update()  # Update the UI to show the question and options

            root.wait_variable(answer_var)  # Wait for the user to select an answer

            if answer_var.get() == q['correct_option']:
                score += 1
...
# Function to display the quiz and recommendations
def start_quiz():
    course_interest = course_var.get()

    score = ask_questions(course_interest)

    if score is None:
        messagebox.showerror("Error", "No questions available for this course.")
        return

    user_skill_level = determine_skill_level(score)
    skill_level_text.set(f"Based on your score, you are classified as {user_skill_level.capitalize()}.")

    recommended_modules = recommend_learning_pathway(user_skill_level, course_interest)
    if isinstance(recommended_modules, str):
        recommendation_text.set(recommended_modules)
    else:
        recommendation_text.set(recommended_modules.to_string(index=False))

# Initialize the Tkinter window
root = tk.Tk()
root.title("Course Recommendation System")

# Variables for dropdown, answer selection, and UI display
course_var = tk.StringVar()
answer_var = tk.StringVar()
question_text = tk.StringVar()
options_list = tk.StringVar()
skill_level_text = tk.StringVar()
recommendation_text = tk.StringVar()

# Create dropdown for course selection
ttk.Label(root, text="Select a Course:").pack(pady=10)
course_dropdown = ttk.Combobox(root, textvariable=course_var, values=course_names, state="readonly")
...
ttk.Label(root, textvariable=question_text).pack(pady=10)
ttk.Label(root, textvariable=options_list).pack(pady=10)

# Dropdown to select an answer
ttk.Label(root, text="Choose an option:").pack(pady=5)
answer_dropdown = ttk.Combobox(root, textvariable=answer_var, values=['a', 'b', 'c'], state="readonly")
answer_dropdown.pack(pady=10)

# Display skill level after the quiz
ttk.Label(root, textvariable=skill_level_text).pack(pady=20)

# Display recommended modules
ttk.Label(root, text="Recommended Modules:").pack(pady=10)
ttk.Label(root, textvariable=recommendation_text).pack(pady=10)
